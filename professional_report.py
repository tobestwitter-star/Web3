...
def _registry_for(finding: Dict[str, Any], analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    value = registry.get(str(finding.get("id")), registry.get(finding.get("id"), {}))
...
def build_professional_report(findings: Iterable[Dict[str, Any]], opportunity: Dict[str, Any] | None = None, analysis: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    authorization = analysis.get("authorization") or {"required": True, "confirmed": bool(analysis.get("authorization_confirmed")), "evidence": scope}
    ...
    for f in findings or []:
        ...
        if isinstance(raw_reproduction, dict): reproduction=dict(raw_reproduction)
        else: reproduction={"malformed":raw_reproduction}
        if not reproduction: reproduction={"evidence":None,"status":"not demonstrated","reproducibility_score":f.get("reproducibility")}
        else:
            reproduction.setdefault("evidence", f.get("execution_evidence"))
            reproduction.setdefault("reproducibility_score", f.get("reproducibility"))
            reproduction.setdefault("status", "observed execution evidence" if f.get("execution_evidence") else "not demonstrated")
        reports.append({
            ...
            "vulnerability_summary": _first(f,"summary","description",default=None), "affected_protocol": _first(f,"protocol",default=opp.get("name")),
            "affected_contract": _first(f,"contract",default=None), "affected_contracts": _list(f.get("contracts") or f.get("affected_contracts")),
            "affected_function": _first(f,"function",default=None), "affected_functions": _list(f.get("functions") or f.get("affected_functions")),
            "source_locations": _list(f.get("location") or f.get("source_locations") or f.get("file")),
            "attack_path": _list(f.get("attack_paths") or f.get("attack_path")), "attack_path_uncertain": f.get("attack_path_uncertain"),
            "root_cause": f.get("root_cause"), "exploitability_reasoning": _first(f,"exploitability_reasoning","exploitability","attack_scenario"),
            "reproduction": reproduction, "engine_evidence": _structured_engine_evidence(f,registry),
            ...
            "conflicting_evidence": _conflicts(registry), "economic_impact": registry.get("economic_analysis") or f.get("economic_analysis") or f.get("economic_impact"),
            "scope_evidence": f.get("scope_evidence") or scope, "authorization_evidence": authorization, "historical_context": f.get("historical_context") or [],
            ...
            "semantic_similarity_leads": [m for m in matches if isinstance(m,dict) and m.get("match_type")=="semantic_similarity_lead"], "duplicate_status": duplicate_status,
            "confidence": f.get("cross_tool_confidence",f.get("confidence")), "validated_by_multiple_tools": f.get("validated_by_multiple_tools"),
            "limitations": _list(f.get("limitations") or ["Finding remains unverified and requires independent human reproduction."]), "remediation": f.get("remediation"),
            "evidence_provenance": registry.get("provenance") or _list(f.get("evidence_provenance") or f.get("source_attribution")), "review_status": STATUS,
        })
    return {"report_version":"1.1","report_type":"professional_bounty_report","review_status":STATUS,"do_not_auto_submit":True,"human_review_only":True,
            ...
            "scope_and_authorization":{"scope":scope,"authorization":authorization},"findings":reports,
