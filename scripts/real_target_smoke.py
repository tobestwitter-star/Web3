...
def run(cmd,cwd=None,timeout=900):
    p=subprocess.run(cmd,cwd=cwd,text=True,capture_output=True,timeout=timeout);return {"returncode":p.returncode,"stdout":p.stdout[-12000:],"stderr":p.stderr[-12000:]}
def main():
    scope=fetch(ENS_SCOPE); info=fetch(ENS_INFO)
    if "Smart Contracts" not in scope or "Maximum Bounty" not in info:raise RuntimeError("ENS public scope evidence could not be verified")
    discovery=PublicProgramDiscovery(); discovered,diagnostics=discovery.discover_public_indexes(); ens=[x for x in discovered if x.name.lower().startswith("ens") and x.source.startswith("immunefi")]
    if not ens:
