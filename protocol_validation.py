...
def evidence_fingerprint(observation: Dict[str, Any]) -> str:
    ...
    material = {"engine": observation.get("engine") or execution.get("engine"), "finding_id": observation.get("finding_id") or execution.get("finding_id"), "hypothesis_id": observation.get("hypothesis_id"), "invariant": observation.get("security_invariant"), "command": execution.get("command"), "returncode": execution.get("returncode"), "stdout_sha256": execution.get("stdout_sha256"), "stderr_sha256": execution.get("stderr_sha256"), "trace": observation.get("trace") or observation.get("traces"), "coverage": observation.get("coverage"), "security_assertion": observation.get("security_assertion"), "invariant_result": observation.get("invariant_result")}
...
def _function_for_finding(finding: Dict[str, Any], protocol_map: Dict[str, Any]) -> Dict[str, Any] | None:
    target, contract, location = str(finding.get("function") or ""), str(finding.get("contract") or ""), str(finding.get("location") or "")
    for fn in protocol_map.get("functions", []) or []:
        ...
        if location and location.startswith(str(fn.get("file"))) and int(finding.get("line") or 0) == int(fn.get("line") or 0): return fn
    return None


def _matching_path(finding: Dict[str, Any], attack_paths: Iterable[Dict[str, Any]]) -> Dict[str, Any] | None:
    contract, function = str(finding.get("contract") or ""), str(finding.get("function") or "")
    wanted = f"{contract}.{function}" if contract and function else function
    for path in attack_paths or []:
        if wanted and path.get("entry_point") == wanted: return path
        if function and str(path.get("entry_point", "")).split(".")[-1] == function: return path
    return None
...
def build_validation_plan(finding: Dict[str, Any], protocol_map: Dict[str, Any] | None = None, attack_paths: Iterable[Dict[str, Any]] | None = None) -> Dict[str, Any]:
    ...
    category, title = str(finding.get("category") or "unknown").lower(), str(finding.get("title") or "candidate")
    ...
    state = {"entry_function": target, "before": finding.get("initial_state") or "record relevant balances, roles, oracle values, state variables and protocol configuration", "during": risk_signals, "after": finding.get("post_state") or "record affected balances, roles, state variables and externally observable protocol state"}
    sequence = list((path or {}).get("sequence", [])) or [{"step":1,"actor":"attacker","action":"establish attacker preconditions and initial state"},{"step":2,"actor":"attacker","action":f"invoke {target} using only in-scope test inputs"},{"step":3,"actor":"protocol","action":"apply the relevant state transitions and external calls"},{"step":4,"actor":"observer","action":"evaluate the security invariant against post-state and trace"}]
    ...
    if path_reachability in ("unreachable", "blocked", "inaccessible") or (path and path.get("reachable") is False): reachability_status = "unreachable"
    elif fn and path: reachability_status = "mapped"
    elif fn or path: reachability_status = "partial"
    else: reachability_status = "unresolved"
    reachability = {"entry_point": target, "source_mapped": bool(fn), "path_mapped": bool(path), "status": reachability_status, "requires_authorized_local_execution": True, "live_target_allowed": False}
    economics = finding.get("economic_analysis") or {}
    economic_consequence = {"status": economics.get("status", "unquantified") if isinstance(economics, dict) else "unquantified", "analysis": economics, "requirement":"quantify only from source-derived balances/prices/fees or reproduced local evidence; never infer from scanner severity"}
    return {"finding_id":finding.get("id"),"title":title,"attacker_preconditions":preconditions,"protocol_state":state,"state_transitions":[s.get("action",s) if isinstance(s,dict) else s for s in sequence],"attack_sequence":sequence,"security_invariant":invariant,"expected_violation":expected,"affected_state":affected,"reachability_constraints":reachability,"economic_consequence":economic_consequence,"risk_signals":risk_signals,"protocol_mapping":{"function":fn,"attack_path":path},"status":STATUS,"validation_only":True}


def validate_plan(plan: Dict[str, Any]) -> List[str]:
    errors = [k for k in REQUIRED_FIELDS if not plan.get(k)]
    seq, reach = plan.get("attack_sequence"), plan.get("reachability_constraints")
...
def correlate_execution_evidence(observations: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    valid = [o for o in obs if isinstance(o, dict) and (o.get("finding_id") or o.get("hypothesis_id"))]
    malformed = [o for o in obs if o not in valid]
