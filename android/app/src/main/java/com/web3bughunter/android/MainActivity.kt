...
class MainVm(private val api: Api): ViewModel() {
    ...
    fun analyze(name:String,code:String) = run { require(name.isNotBlank() && code.isNotBlank()){ "Protocol name and authorized source are required" }; val d=api.call("/api/analyze-advanced","POST",JSONObject().put("protocol_name",name).put("code",code).put("authorized_scope_verified",true)); val a=d.optJSONArray("detailed_reports")?:JSONArray(); findings=(0 until a.length()).map { val f=a.getJSONObject(it); Finding(f.optString("finding_id"),f.optString("vulnerability"),f.optString("severity"),f.optDouble("confidence"),f.optString("detailed_report"),f.optString("status",HUMAN_REVIEW)) }; tab="findings" }
...
@Composable fun Opps(vm:MainVm){ Column(Modifier.padding(12.dp)){ Button(onClick={vm.discover()},enabled=!vm.busy,modifier=Modifier.fillMaxWidth()){Text(if(vm.busy)"Loading…" else "Discover & Rank")}; Spacer(Modifier.height(8.dp)); LazyColumn{items(vm.opps){o-> Card(Modifier.fillMaxWidth().padding(vertical=5.dp)){Column(Modifier.padding(12.dp)){Text(o.name,fontWeight=FontWeight.Bold);Text("${o.source.uppercase()} • ${o.status}");Text("Score ${o.score}/100 • max bounty $${o.bounty.toInt()}");Text("Likelihood ${(o.likelihood*100).toInt()}% • difficulty ${(o.difficulty*100).toInt()}%");Button(onClick={vm.select(o)},enabled=!vm.busy){Text("Select for investigation")}}}}} } }

@Composable fun Analyze(vm:MainVm){ var name by remember{mutableStateOf("")};var code by remember{mutableStateOf("")};Column(Modifier.padding(12.dp)){Text("Authorized protocol analysis",style=MaterialTheme.typography.titleLarge);Text(HUMAN_REVIEW,fontWeight=FontWeight.Bold);Text("Only analyze targets whose scope you have verified. Nothing is submitted automatically.",modifier=Modifier.padding(vertical=8.dp));OutlinedTextField(name,{name=it},label={Text("Protocol name")},modifier=Modifier.fillMaxWidth());Spacer(Modifier.height(8.dp));OutlinedTextField(code,{code=it},label={Text("Authorized Solidity/source code")},modifier=Modifier.fillMaxWidth().height(220.dp));Spacer(Modifier.height(8.dp));Button(onClick={vm.analyze(name,code)},enabled=!vm.busy,modifier=Modifier.fillMaxWidth()){Text(if(vm.busy)"Analyzing…" else "Run analysis")}}
}

@Composable fun Findings(vm:MainVm){Column(Modifier.padding(12.dp)){Text("Findings — Human Verification Queue",style=MaterialTheme.typography.titleLarge);Text(HUMAN_REVIEW,fontWeight=FontWeight.Bold);LazyColumn{items(vm.findings){f->Card(onClick={vm.selected=f}, modifier=Modifier.fillMaxWidth().padding(vertical=5.dp)){Column(Modifier.padding(12.dp)){Text(f.title,fontWeight=FontWeight.Bold);Text("${f.severity.uppercase()} • confidence ${(f.confidence*100).toInt()}%");Text(f.status)}}}}};vm.selected?.let{f->AlertDialog(onDismissRequest={vm.selected=null},confirmButton={TextButton({vm.selected=null}){Text("Close")}},title={Text(f.title)},text={Column{Text(HUMAN_REVIEW,fontWeight=FontWeight.Bold);Text(f.report)}})}}
