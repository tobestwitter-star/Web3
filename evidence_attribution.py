...
class SourceAttributor:
    ...
    def __init__(self, code: str):
        self.code=code; self.lines=code.splitlines()
        self.files=self._spans(self.FILE_RE,False); self.contracts=self._spans(self.CONTRACT_RE,True)
        self.functions=self._spans(self.FUNCTION_RE,True); self.modifiers=self._spans(self.MODIFIER_RE,True)
    ...
    def _span_at(self,spans,pos):
        hits=[s for s in spans if s['start']<=pos<s['end']]
        return min(hits,key=lambda s:s['end']-s['start']) if hits else None
    ...
    def context_at(self,pos,evidence_start=None,evidence_end=None):
        ...
        fn=self._span_at(self.functions,pos); mod=self._span_at(self.modifiers,pos); container=self._span_at(self.contracts,pos); chosen=fn or mod
    ...
    def evidence_position(self,location='',evidence=None,preferred_patterns=()):
        ...
        for item in items:
            ...
            if len(text)>=24:
                pos=self.code.find(text)
                if pos>=0:return pos,pos+len(text)
        joined=' '.join(str(x or '') for x in items)
        for pat in preferred_patterns:
            em=re.search(pat,joined,re.I)
            if em:
                pos=self.code.find(em.group(0))
                if pos>=0:return pos,pos+len(em.group(0))
        return None,None

    def attribute(self,finding):
        pos,eend=self.evidence_position(finding.get('location'),finding.get('evidence'),(
            r'\b\w+\s*\.\s*(?:call|send|transfer)\b',
            r'\b(?:delegatecall|ecrecover)\s*\(',
            r'\b(?:state|status|owner|admin|balance\w*|shares|debt|nonce)\w*\s*(?:=|\+=|-=)',))
        ctx=self.context_at(pos,evidence_start=pos,evidence_end=eend) if pos is not None else SourceContext(None,None,None,None,None,None,'uncertain','No reliable source position was available.')
