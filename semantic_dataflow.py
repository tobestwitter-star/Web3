...
class SemanticDataflow:
    def _functions(self, code):
        ...
        return out
    def _finding(self,cat,title,sev,code,pos,desc,evidence,conf=.78): return {'title':title,'severity':sev,'category':cat,'location':f"Source evidence near line {code[:pos].count(chr(10))+1}",'description':desc,'confidence':conf,'evidence':[evidence[:2200]],'status':STATUS}
    ...
    def _state_write(self,b): return re.search(r'\b(?:balance|balances|pending|shares|debt|credit|credits|state|status|phase|mode|owner|admin|controller|governor|implementation|logic|total|reserve|cash|quota|nonce)\w*(?:\s*\[[^\]]+\])?\s*(?:=|\+=|-=|\*=|\+\+|--)',b,re.I)
    def analyze(self,code,name='target'):
        fs=[];funcs=self._functions(code);interfaces={};iface_vars={}
        for im in re.finditer(r'interface\s+(\w+)\s*\{(.*?)\}',code,re.I|re.S): interfaces[im.group(1)]={m.group(1) for m in re.finditer(r'function\s+(\w+)\s*\([^)]*\)\s*([^;]*);',im.group(2),re.I) if not re.search(r'\b(?:view|pure)\b',m.group(2),re.I)}
        ...
        for m,n,a,h,b in funcs:
            ...
            if calls:
                ...
                if sw and not re.search(r'\b(?:nonReentrant|lock(?:ed)?\s*=\s*true)\b',h+' '+b,re.I):
                    ...
                    break
        for m,n,a,h,b in funcs:
            if re.search(r'\becrecover\s*\(',b,re.I):
        ...
        if authority_vars:
            for m,n,a,h,b in funcs:
                ...
                if self._state_write(b) and not self._auth(h,b) and not any(re.search(r'\b'+re.escape(v)+r'\s*\.',b,re.I) for v in authority_vars): fs.append(self._finding('access_control','Cross-contract authorization check is not enforced on the state-changing path','high',code,m.start(),'An authority-like external contract is queried only outside the sensitive mutation path; the state-changing function does not enforce the cross-contract authorization invariant.',b,.86))
        for m,n,a,h,b in funcs:
        ...
        for m,n,a,h,b in funcs:
            oracle_call=re.search(r'\b(?:getPrice|latest|latestPrice|latestAnswer|latestRoundData|read|consult|spotPrice|twap)\s*\(',b,re.I)
            if not oracle_call: continue
            assigned=re.search(r'\b(?:uint\w*\s+)?(\w+)\s*=\s*\w+\.(?:getPrice|latest|latestPrice|latestAnswer|latestRoundData|read|consult|spotPrice|twap)\s*\(',b,re.I)
        ...
        for m,n,a,h,b in funcs:
            for dm in re.finditer(r'\b(\w+)\s*=\s*([^;\n]*?/[^;\n]+)',b,re.I):
                expr=dm.group(2)
                if '*' in expr.split('/')[0] or re.search(r'\b(?:amount|value|units|shares|rate|price|totalShares|assets)\b',expr,re.I): fs.append(self._finding('precision','Potential truncation before value scaling','medium',code,m.start(),'A value is divided after a value-bearing multiplication, creating a potential precision-loss surface that needs boundary-value reproduction.',expr,.78));break
            for cm in re.finditer(r'\b[\w\[\].]+\s*(?:\+=|-=)\s*[^;\n]*/\s*\d+',b,re.I): fs.append(self._finding('precision','Precision loss in compound value conversion','medium',code,m.start(),'A value-bearing compound assignment performs integer division directly, creating truncation risk that should be reproduced at boundary values.',cm.group(0),.8));break
            ratio_assign=re.search(r'\b(?:uint\w*\s+)?(payout|out|payment|amountOut)\s*=\s*\w+\s*\*\s*(\d+)\s*/\s*(\d+)\s*;',b,re.I)
            if ratio_assign and int(ratio_assign.group(2))>int(ratio_assign.group(3)) and re.search(r'\b(?:transfer|send|call)\s*\([^;\n]*\b'+re.escape(ratio_assign.group(1))+r'\b',b,re.I): fs += [self._finding('asset_flow','Asset payout exceeds accounted unit','high',code,m.start(),'The withdrawal path transfers a scaled-up payout while the caller balance is debited in the original unit.',b,.9),self._finding('accounting','Accounting conservation mismatch','high',code,m.start(),'Recorded units and transferred assets use different quantities on the withdrawal path.',b,.88)]
