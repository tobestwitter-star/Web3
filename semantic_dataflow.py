...
class SemanticDataflow:
    ...
    def analyze(self,code,name='target'):
        ...
        for m in re.finditer(r'\bdelegatecall\s*\(',code,re.I):
            before=code[max(0,m.start()-700):m.start()]; fn=None
            for fm,nn,aa,hh,bb in funcs:
                if fm.start()<=m.start()<=fm.end()+len(bb): fn=(hh,bb);break
            governed=self._auth(*(fn or ('','')),before)
            target_caller=bool(fn and re.search(r'\b(?:msg\.sender|next|target|implementation)\b[^;\n]{0,120}(?:delegatecall|=)',fn[1],re.I))
            if not governed or target_caller:
                desc='A delegatecall creates an execution-context trust boundary. The implementation target is not visibly governed in this path; local reproduction is required.'
