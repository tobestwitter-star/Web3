...
class ProtocolMapper:
 def map(self,source_dir):
  root=Path(source_dir);contracts=[];functions=[];edges=[];files=[];all_names=[]
  ...
  for path in root.rglob('*.sol'):
   ...
   for m in SOL_RE.finditer(text):
    ...
    for fn in FN_RE.finditer(text):
     body=text[fn.end():min(len(text),fn.end()+1400)];functions.append({'contract':name,'name':fn.group(1),'file':rel,'line':text[:fn.start()].count('\n')+1,'signature':fn.group(2).strip(),'modifiers':fn.group(3).strip(),'external_call':bool(CALL_RE.search(body)),'value_flow':bool(VALUE_RE.search(fn.group(1)+fn.group(2)+body[:700])),'oracle_dependency':bool(ORACLE_RE.search(body[:1200])),'privileged':bool(ROLE_RE.search(fn.group(3))),'payable':bool(re.search(r'\bpayable\b',fn.group(3),re.I)),'state_write':bool(re.search(r'\b(?:state|status|balance|shares|debt|owner|admin|total[A-Za-z]*)\w*\s*(?:\+=|-=|=)',body[:1200],re.I))})
...
class AttackPathEngine:
 def paths(self,protocol_map,max_paths=60):
  paths=[]
  for f in protocol_map.get('functions',[]):
   risk=[]
   ...
   if f.get('state_write'):risk.append('state transition')
   if not risk:continue
   prerequisite='attacker-controlled caller can reach the entry point'
   if f.get('privileged'):prerequisite='attacker must satisfy or bypass the observed privileged boundary'
   vulnerable=' '.join(risk)+' requires invariant/state-order validation'
   outcome='attacker-controlled outcome requires local state/trace reproduction'
   paths.append({'sequence':[{'step':1,'actor':'attacker','action':'satisfy prerequisite and call entry point','contract':f['contract'],'function':f['name'],'file':f['file'],'line':f['line']},{'step':2,'actor':'protocol','action':'execute vulnerable state transition','condition':vulnerable},{'step':3,'actor':'attacker','action':'receive/control resulting value or state','condition':outcome}], 'entry_point':f'{f["contract"]}.{f["name"]}','risk_signals':risk,'attacker_privilege':'role/admin' if f.get('privileged') else 'user','preconditions':[prerequisite],'state_changes':['Confirm storage writes before/after external calls and value movement.'],'asset_flow':'Source-derived asset movement is a candidate only until a local trace establishes amounts.','expected_gain':'not quantified until balances/prices/fees are supplied by source or reproduction','protocol_loss':'not quantified until balances/prices/fees are supplied by source or reproduction','why_checks_fail':'hypothesis only; reproduce the complete sequence and security invariant locally','status':'UNVERIFIED — HUMAN REVIEW REQUIRED'})
