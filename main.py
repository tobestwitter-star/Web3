...
def queue_fill():
 d=request.get_json(silent=True) or {};ops=d.get('opportunities') if d.get('opportunities') is not None else store.list(200);queue.fill(ops,int(d.get('limit',50)));return jsonify({'status':'queued','queue':queue.list(200)})
