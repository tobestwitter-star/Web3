...
def research_resolve():
 d=request.get_json(silent=True) or {};op=d.get('opportunity') or {};e=str(d.get('public_evidence',''));scope=research.scope.resolve(op,e);build=research.build.detect(str(d.get('source_dir',''))) if d.get('source_dir') else {'primary':None,'detected':[],'not_run':True};from target_resolution import TargetMap;return jsonify(TargetMap().build(scope,build))
...
def research_analyze():
 ...
 if not d.get('authorized_scope_verified'):return jsonify({'error':'authorized_scope_verified must be true'}),403
 return jsonify(research.analyze_local(str(d.get('source_dir','')),str(d.get('protocol_name','authorized-target')),d.get('source_code'),True,d.get('tools')))
...
def ranked_opportunities():return jsonify({'opportunities':store.list(min(int(request.args.get('limit',50)),200)),'selection_basis':'Expected reward + finding likelihood + severity + attack surface + source availability + difficulty + research-time efficiency + competition risk'})
