...
def test_structured_engine_observations_survive_correlation():
    raw = {'id':'sl-1','title':'Reentrancy','severity':'high','confidence':0.9,'location':'src/Vault.sol:40',
           'evidence':[{'source_mapping':{'filename_relative':'src/Vault.sol','lines':[40,41]}}]}
...
def test_report_prefers_authoritative_registry_over_flattened_fields():
    finding={'id':'f-1','title':'Candidate','severity':'high','engine_observations':[{'engine':'fake','evidence':['flattened']}],
             'execution_evidence':{'status':'executed','trace':'flattened'}}
    registry={'f-1':{'engine_observations':[{'engine':'forge','structured_result':{'status':'completed','command':['forge','test']}}],
                     'execution_metadata':[{'status':'completed','command':['forge','test'],'duration_seconds':1.2}],
                     'provenance':[{'engine':'forge','version':'forge 1.0','stdout_sha256':'abc'}],
                     'reproduction':{'status':'assertion_failed','trace':{'pc':[1,2]},'inputs':{'amount':7}},
                     'symbolic':{'tool':'halmos','evidence_level':'symbolic_counterexample'},
                     'invariant':{'tool':'forge-invariant','evidence_level':'no_invariant_failure_observed'},
                     'conflicts':[{'engines':['slither','forge'],'reason':'one reports risk, one has no failing regression'}]}}
...
def test_pipeline_registry_contains_stage1_and_candidate_provenance_without_fabrication():
    p=ResearchPipeline()
    orchestration={'stage1':{'results':[{'tool':'slither','version':'0.1','result':{'status':'completed'},
        'evidence_provenance':{'engine':'slither','command':['slither','.'],'stdout_sha256':'s'},
        'findings':[{'id':'f-3','title':'Candidate','evidence':[{'element':'x'}]}]}]}}
    ...
    assert any(x['engine']=='forge' for x in reg['provenance'])
