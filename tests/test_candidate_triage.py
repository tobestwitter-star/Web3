...
def test_view_state_mutation_claim_is_false_positive():
    out = triage_finding({
        'title': 'Sensitive privilege mutation lacks authorization',
...
def test_pure_reentrancy_claim_is_false_positive():
    out = triage_finding({
        'title': 'Reentrancy: external call precedes state update',
        'source_attribution': {'mutability': 'pure'},
    })
    assert out['triage_classification'] == 'false_positive'
    assert out['bounty_candidate'] is False
    assert out['triage_reason']
...
def test_mutating_candidate_requires_reproduction():
    out = triage_finding({
        'title': 'Reentrancy: external call precedes state update',
        'source_attribution': {'mutability': 'nonpayable'},
    })
