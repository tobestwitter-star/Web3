...
Evidence-gated source analysis. Findings remain unverified until a human validates
scope, reachability, exploitability and impact in an authorized local environment.
"""
...
class AdvancedWeb3Analyzer:
 ...
 def _state_write_after(self,body,pos):
  return bool(re.search(r'\b(?:balance|balances|shares|debt|state|status|owner|admin|total[A-Za-z]*|allowance)\w*\s*(?:=|\+=|-=|\*=)',body[pos:],re.I))
 ...
 def _specialized(self,code):
  ...
  # Cryptographic authorization must lead to a privileged/value-bearing mutation.
  ...
  # Reentrancy requires an external boundary followed by a sensitive write; CEI and mutexes are safe.
  for m,name,args,head,body in funcs:
   if re.search(r'\bnonReentrant\b',head,re.I) or re.search(r'\block(?:ed)?\s*=\s*true\b',head+' '+body,re.I):continue
   calls=list(re.finditer(r'(?:\b\w+\s*\.\s*(?:call|send|transfer)\b|\b(?:I\w+|[A-Z]\w*)\s*\([^;\n]{0,260}\))',body,re.I))
   for call in calls:
    if self._state_write_after(body,call.end()):
     ev=body[:1800];fs += [self._finding('reentrancy','Reentrancy: external callback precedes state update','critical','reentrancy',code,m.start(),'An external call/callback occurs before a relevant state write and no obvious reentrancy mutex is present. This is an attack-path candidate, not confirmation.',ev,'Potential repeated or inconsistent state/value movement; local attacker callback reproduction is required.',.88),self._finding('callback','Unsafe callback flow before accounting/state mutation','high','callback',code,m.start(),'An external callback executes before the caller finalizes sensitive accounting/state.',ev,'Potential cross-function or callback reentrancy; impact requires local reproduction.',.82),self._finding('external_call','Security-sensitive external call ordering','high','external_call',code,m.start(),'A cross-contract call precedes a relevant state mutation in the same callable path.',ev,'Potential callback-controlled state manipulation; validate locally.',.78)]
     break
  # Sensitive operations include common value/privilege verbs such as mintCredit.
  ...
  # Explicit ERC-style receiver callback before accounting.
  for m,name,args,head,body in funcs:
   cb=re.search(r'\b(?:\w+\s*\.\s*)?(?:onTokenReceived|onERC\w+|tokensReceived)\s*\(',body,re.I)
 ...
 def _legacy_signals(self,code):
  ...
  # Accounting is promoted only when a concrete conservation mismatch is visible.
