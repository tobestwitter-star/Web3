...
class MatureEngineRunner:
 ...
 def inventory(self)->Dict[str,Dict[str,Any]]:
  return {"slither":self._engine("slither","AGPL-3.0-or-later","crytic/slither"),"forge":self._engine("forge","Apache-2.0 OR MIT","foundry-rs/foundry")}
 @staticmethod
 ...
 def _slither_findings(result):
  ...
  for d in detectors if isinstance(detectors,list) else []:
   ...
   impact=str(d.get("impact") or "informational").lower();conf=str(d.get("confidence") or "medium").lower()
   first=locs[0] if locs else {};line=(first.get("lines") or [None])[0]
