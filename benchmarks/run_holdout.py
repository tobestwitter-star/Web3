...
def run_fixture(p,fx):
 ...
 try:
  ...
  fs=a.get('correlated_findings',[]);r.update({'analysis_status':a.get('status'),'attack_paths':a.get('attack_paths',[]),'findings':fs,'tool_results':a.get('tool_results',{}),'symbolic':a.get('symbolic_validation',{}),'invariants':a.get('invariant_validation',{}),'upgrade_surface':a.get('upgrade_surface',{})})
  hit=set();
  for f in fs:
   for e in fx['expected']:
    if expected_hit(f,[e]):hit.add(e)
  r['detected']=sorted(hit);r['missed']=[e for e in fx['expected'] if e not in hit];r['false_positives']=len(fs) if fx['safe'] else 0
  r['evidence_quality']={'finding_count':len(fs),'with_source_location':sum(bool(f.get('location')) for f in fs),'with_evidence':sum(bool(f.get('evidence')) for f in fs),'with_attack_path':sum(bool(f.get('attack_paths')) for f in fs),'with_execution_evidence':sum(bool(f.get('execution_evidence')) for f in fs),'all_unverified':all(f.get('status')==STATUS for f in fs)}
  r['reproducibility']=[f.get('reproducibility',0) for f in fs];r['reports']=build_report(fs,opp);r['report_count']=len(r['reports'].get('findings',[]));r['tool_coverage']={x.get('name'):('skipped' if x.get('skipped') else 'executed') for x in a.get('tool_results',{}).get('results',[])};r['elapsed_seconds']=round(time.perf_counter()-start,3);r['result']='ok'
...
def main():
 ...
 payload={'benchmark':'Web3 BugHunter unseen holdout','version':m['version'],'summary':summary,'fixtures':rs,'status':STATUS};out=ROOT/'benchmarks/results';out.mkdir(exist_ok=True);(out/'holdout.json').write_text(json.dumps(payload,indent=2,default=str));(out/'holdout.md').write_text('# Web3 BugHunter Unseen Holdout\n\nStatus: '+STATUS+'\n\n## Summary\n'+''.join(f'\n- **{k}**: {v}' for k,v in summary.items())+'\n',encoding='utf-8');print(json.dumps(summary,indent=2));return 0
