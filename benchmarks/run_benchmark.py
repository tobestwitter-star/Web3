...
def run_fixture(pipeline,fixture,root):
    name=fixture['name']; source=root/fixture['file']
    start=time.perf_counter(); record={'fixture':name,'expected':fixture['expected'],'safe':fixture['safe'],'stages':{},'status':STATUS}
    try:
        ...
        record['stages']['acquisition']={'mode':'local_fixture','ok':True,'authorized':True}
        result=pipeline.analyze_local(str(root),name,code,True,['slither','aderyn','wake'],opportunity)
        record['stages']['analysis_status']=result.get('status')
        record['stages']['build']=result.get('build')
        record['stages']['protocol_mapping']=result.get('protocol_map',{})
        record['stages']['attack_paths']=result.get('attack_paths',[])
        record['stages']['external_tools']=result.get('tool_results',{})
        record['stages']['symbolic']=result.get('symbolic_validation',{})
        record['stages']['invariants']=result.get('invariant_validation',{})
        record['stages']['upgrade_surface']=result.get('upgrade_surface',{})
        ...
        record['missed']=[x for x in fixture['expected'] if x not in detected]
...
def summarize(records):
    ...
    expected=sum(len(r['expected']) for r in vuln);detected=sum(len(r.get('detected',[])) for r in vuln);missed=expected-detected
    fp=sum(r.get('false_positives',0) for r in safe)
    precision=detected/(detected+fp) if detected+fp else 0.0
...
def main():
    ...
    fixture_root=ROOT/'benchmarks/fixtures';pipeline=ResearchPipeline();records=[]
    for fixture in manifest['fixtures']:
        records.append(run_fixture(pipeline,fixture,fixture_root))
    ...
    out=ROOT/'benchmarks/results';out.mkdir(parents=True,exist_ok=True)
    (out/'benchmark.json').write_text(json.dumps(payload,indent=2,default=str),encoding='utf-8')
    lines=['# Web3 BugHunter Offline Benchmark','',f"Status: {STATUS}",'', '## Summary', '']
    for k,v in summary.items(): lines.append(f'- **{k}**: {v}')
    lines += ['', '## Fixture results','']
    for r in records:
        lines.append(f"### {r['fixture']}")
        lines.append(f"- result: {r['result']}")
        lines.append(f"- expected: {', '.join(r['expected']) or 'none'}")
        lines.append(f"- detected: {', '.join(r.get('detected',[])) or 'none'}")
        lines.append(f"- missed: {', '.join(r.get('missed',[])) or 'none'}")
        lines.append(f"- false positives: {r.get('false_positives',0)}")
        lines.append(f"- elapsed_seconds: {r.get('elapsed_seconds')}")
