...
def _normalize_halmos(result):
    raw=result.get("json_output","")
    try:
        data=json.loads(raw) if raw else {}
        tests=[t for rows in data.get("test_results",{}).values() for t in rows]
        counterexamples=sum(int(t.get("num_models",0)) for t in tests)
        passes=sum(int(t.get("exitcode",1))==0 for t in tests)
        failures=sum(int(t.get("exitcode",1))!=0 for t in tests)
        if counterexamples and passes and failures:
            result["semantic_status"]="counterexample_found_expected_for_vulnerable_fixture"
            result["structured_symbolic_evidence"]={"counterexample_count":counterexamples,"passing_safe_tests":passes,"failing_security_assertions":failures,"tests":tests}
