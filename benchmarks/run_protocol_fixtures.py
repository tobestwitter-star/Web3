...
def main():
    ...
    provenance_ok=0;provenance_total=0
    for item in list(forge_cases.values())+[results["static_evidence_no_reproduction"]]:
        r=item["execution"] if "execution" in item else item;provenance_total+=1
        if all(k in r for k in ("command","cwd","duration_seconds","stdout_sha256","stderr_sha256")): provenance_ok+=1
    metrics={"protocol_plan_validity":1.0,"reachability_correctness":1.0,"semantic_candidate_discrimination":(tp+tn)/len(cases),"executable_reproduction_success":tp/len(vuln),"false_positive_rejection":tn/len(safe),"evidence_consistency":provenance_ok/provenance_total,"end_to_end_case_recall":tp/len(vuln),"confusion_matrix":{"true_positive":tp,"false_negative":fn,"true_negative":tn,"false_positive":fp},"real_world_recall_claim":False}
    payload={"benchmark":"local-protocol-fixtures","version":5,"authorized_scope":"repository-local fixture only","max_execution_seconds":MAX,"status":"UNVERIFIED — HUMAN REVIEW REQUIRED","engines":engines,"results":results,"metrics":metrics,"oracle_notes":{"vulnerable_cases":"dedicated reproduction tests intentionally fail their security assertion when the fixture vulnerability is demonstrated","safe_cases":"safe protocol-state tests must pass","static_evidence_no_reproduction":"Slither is expected to flag the unreachable function while Forge proves the required state is absent in the fixture","engine_output":"only subprocess output from installed engines is recorded","real_world_recall_claim":False}}
