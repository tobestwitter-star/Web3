...
def main():
    ...
    if engines["halmos"]["available"]:
        fd,path=tempfile.mkstemp(prefix="halmos-fixture-",suffix=".json");Path(path).unlink(missing_ok=True)
        results["halmos"]=run(["halmos","--json-output",path]);results["halmos"]["json_output_sha256"]=sha(Path(path).read_text(encoding="utf-8")) if Path(path).exists() else None;Path(path).unlink(missing_ok=True)
    else: results["halmos"]={"status":"unavailable","reason":"halmos not installed"}
    results["ityfuzz"]=run(["ityfuzz","evm","-m","src/ItyFuzzDeployment.sol:ItyFuzzDeployment","--","forge","build"]) if engines["ityfuzz"]["available"] else {"status":"unavailable","reason":"ityfuzz not installed"}
