...
QUESTION_MAP = {
    "reentrancy": {"slither", "forge", "ityfuzz"}, "state_machine": {"forge", "halmos", "medusa", "echidna", "ityfuzz"},
    "symbolic": {"halmos"}, "dependencies": {"osv-scanner"}, "secrets": {"gitleaks"}, "verification": {"sourcify"},
    "execution_path": {"forge", "ityfuzz"}, "independent_static": {"wake"}, "invariant": {"forge", "medusa", "echidna"},
    "broad": {"slither", "forge"},
}
...
class SecurityEngineOrchestrator:
    ...
    def inventory(self) -> List[Dict[str, Any]]:
        ...
        for name, policy in ENGINE_POLICY.items():
            if name not in base:
                ...
                base[name] = {"name": name, "binary": shutil.which(binary) if binary else None, "available": bool(binary and shutil.which(binary)), "kind": "verification-service" if name == "sourcify" else "external-engine", "license": "service/protocol; no local binary", "project": "ethereum/sourcify" if name == "sourcify" else "unknown"}
    ...
    def _run(self, command: List[str], cwd: str, timeout: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "completed" if p.returncode == 0 else "failed", "returncode": p.returncode, "stdout": p.stdout[-30000:], "stderr": p.stderr[-15000:], "duration_seconds": round(time.monotonic()-started, 3)}
    ...
    def run_stage1(self, source_dir: str, timeout: int = 120, explicit: Iterable[str] | None = None) -> Dict[str, Any]:
        ...
        for name in selected:
            if name == "slither":
                core = self.core.run_slither(source_dir, timeout); results.append({"tool":"slither","stage":1,"question":"broad","result":core.get("execution",{}),"findings":core.get("findings",[]),"integration":core,"review_status":STATUS}); continue
            if name == "forge":
                core = self.core.run_foundry(source_dir, timeout); results.append({"tool":"forge","stage":1,"question":"baseline","result":core.get("execution",{}),"findings":core.get("findings",[]),"integration":core,"review_status":STATUS}); continue
            if name == "osv-scanner": command=["osv-scanner","scan","--format","json","--recursive","."]
            elif name == "gitleaks": command=["gitleaks","detect","--no-banner","--report-format","json","--report-path","-"]
            else: continue
    ...
    def run_for_candidates(self, source_dir: str, findings: List[Dict[str, Any]], timeout: int = 180) -> Dict[str, Any]:
        ...
        for finding in candidates:
            ...
            evidence=[]
            for name in chosen:
                command={"halmos":["halmos"],"ityfuzz":["ityfuzz"],"medusa":["medusa","fuzz"],"echidna":["echidna-test","."]}.get(name)
                if not command: continue
                result=self._run(command,source_dir,min(int(timeout),180)); evidence.append({"tool":name,"stage":ENGINE_POLICY[name]["stage"],"question":question,"result":result,"review_status":STATUS})
