...
def build_report(findings,opportunity):
    ...
    for f in findings or []:
        normalized.append({'title':f.get('title') or f.get('vulnerability') or 'Potential vulnerability','severity':f.get('severity') or f.get('severity_potential') or 'unrated','affected_contracts':f.get('contracts') or ([f.get('contract')] if f.get('contract') else []),'affected_functions':f.get('functions') or ([f.get('function')] if f.get('function') else []),'root_cause':f.get('root_cause') or f.get('description') or f.get('hypothesis',''),'attack_scenario':f.get('attack_scenario') or f.get('hypothesis',''),'prerequisites':f.get('prerequisites') or [],'poc_reproduction':f.get('poc') or f.get('execution_evidence') or f.get('proof_of_concept') or 'Local reproduction required before submission.','evidence':f.get('evidence') or f.get('execution_evidence') or [],'impact':f.get('impact') or f.get('economic_impact') or 'Impact requires human verification.','economic_impact':f.get('economic_analysis') or f.get('economic_impact') or {},'confidence':f.get('confidence',f.get('cross_tool_confidence',0.0)),'remediation':f.get('remediation') or 'Apply the minimal state/authorization/invariant fix and add a regression test.','scope_evidence':scope_evidence,'possible_duplicate_indicators':f.get('possible_duplicate_indicators') or f.get('duplicate_indicators') or [],'status':'UNVERIFIED — HUMAN REVIEW REQUIRED'})
